...
class XddaSupabaseUser extends BaseAuthUser {
  XddaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> xddaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = XddaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
